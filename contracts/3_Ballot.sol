    ...
    */
    function getTokenId(address to) public view returns (MyNft[] memory) {
        ...
        for (uint256 i = 0; i < count[to]; i++) {
            myArray[i] = 
            MyNft(TokenId[to][i + 1],
            NFTMetadata[TokenId[to][i + 1]].mintTime,
            address(this),
            NFTMetadata[TokenId[to][i + 1]].mintArtist,
            NFTMetadata[TokenId[to][i + 1]].artistFeePerAge,
            tokenURI(TokenId[to][i + 1])
            );
        }
        return myArray;
    }
